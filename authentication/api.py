...
def create(request, user: UserIn):
    try:
        user_dict = user.dict()
        referral = None
        if user_dict["referred_by"]:
            try:
                referral = CustomUser.objects.get(
                    referral_code=user_dict["referred_by"]
                )
            except CustomUser.DoesNotExist:
                referral = None
        user_dict.pop("referred_by", None)
        user_model = CustomUser.objects.create_user(
            **user_dict,
            referral_code=generate_referral_code(user_dict["email"]),
...
@router.get(
    "/email-already-exist/{email}", response={200: EmailExistOut, 500: ErrorOut}
)
...
@router.get(
    "/verify-otp/{otp}",
    response={200: AuthOut, codes_4xx: ErrorOut, codes_5xx: ErrorOut},
    auth=AuthBearer(),
)
@transaction.atomic
...
@router.get(
    "personal-information", auth=AuthBearer(), response={200: UserOut, 500: ErrorOut}
)
def get_user_personal_information(request):
    try:
        userId = request.auth["user"]["id"]
        user = (
            CustomUser.objects.select_related("tier2")
            .select_related("tier3")
            .get(id=userId)
        )
        ...
        return 200, {
            ...
            **model_to_dict(user),
            "tier2": (
                model_to_dict(getattr(user, "tier2", None))
                if hasattr(user, "tier2")
                else None
            ),
            "tier3": (
                model_to_dict(getattr(user, "tier3", None))
                if hasattr(user, "tier3")
                else None
            ),
        }
    except Exception as error:
...
def reset_password(request, body: ChangePasswordIn):
    try:
        ...
        if len(data["new"]) < 6:
            return {
                "success": False,
                "msg": "password must be six (6) characters or long",
            }
...
def tier2_verification(
    ...
    try:
        ...
        if UserVerification.objects.filter(user__id=userId).exists():
            return {
                "success": False,
                "msg": "Existing verification details found, contact support",
            }
...
def add_next_of_kin(request, body: NextOfKinSchema):
    try:
        ...
        if NextOfKin.objects.filter(user__id=userId).exists():
            return {
                "success": False,
                "msg": "Existing next of kin details found, contact support",
            }
        user = CustomUser.objects.get(id=userId)
...
@router.get(
    "next-of-kin",
    auth=AuthBearer(),
    response={200: NextOfKinSchemaOut, 404: ErrorOut, 500: ErrorOut},
)
